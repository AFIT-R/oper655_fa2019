...
Analyzing word and trigram frequency grouped by the various independent variables shows how the usage of each word or trigram differs.  The frequency for age, education level, and years in prison are below.  
...
grid.arrange(Ed_word_freq, Ed_trigram_freq, nrow = 2)
...
````{r}
# use this to reorder the years in prison
...
````
2. Topic Modeling
  - Discuss what topic modeling is/what it tells us
...
topic_model_list <- list(topic_model1, topic_model2, topic_model3, topic_model4, topic_model5)

tidy_list <- list()
for(item in topic_model_list){
  tidy(item) %>%  group_by(topic) %>%
  top_n(7) %>%
  ungroup %>%
...
tidy_topic_1 <- tidy(topic_model1)
